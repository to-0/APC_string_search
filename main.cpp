...
Position calculate_beginning_position(Position current, int overall_distance,Arguments &arguments) {
	Position beginning;
	if (arguments.pattern_new_lines != 0) { //multi line pattern
		beginning.column = arguments.rows_columns[0] - arguments.chars_before_first_new_line;
		beginning.row = current.row - arguments.pattern_new_lines + 1;
	}
...
int char_lookup(Arguments &arguments) {
	...
	//buffer.resize(sizeof(arguments.pattern));
	while(true) {
		...
		if (hash == pattern_hash) {
			Position temp = calculate_beginning_position(current_position, overall_distance, arguments);
			...
			if (check_patterns(buffer, arguments) == 1) {
				if (last_left == -1) { //this is the first pattern found so there can't be neighbour from the left
					...
					if (temp.distance - last_position.distance <= arguments.n) { // check the distance between current and last to the left
				...
				last_position = temp; //last position of the pattern to the left
			}
		}
		...
		if (c == '\n') {
			...
			arguments.rows_columns.insert(arguments.rows_columns.begin(), current_position.column + 1); //+1 because we count the newline like it is in the row
...
int main(int argc, char *argv[])
{
	...
	argum.rows_columns.resize(new_lines);
